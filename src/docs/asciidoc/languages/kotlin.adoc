...
----
    ...
    class PersonRepository(private val operator: TransactionalOperator) {
        ...
        private fun findPeople(): Flow<org.sp.Person> {
            // SELECT SQL statement
        }

        private suspend fun updatePerson(person: org.sp.Person): org.sp.Person {
...
As the following example shows, this allows for easy changes to individual properties, even if `org.sp.Person` properties are read-only:

[source,kotlin,indent=0]
----
	data class org.sp.Person(val name: String, val age: Int)

	val jack = org.sp.Person(name = "Jack", age = 1)
