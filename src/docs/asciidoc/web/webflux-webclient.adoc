...
----
	...
	Mono<org.sp.Person> result = client.get()
			.uri("/persons/{id}", id).accept(MediaType.APPLICATION_JSON)
			.retrieve()
			.bodyToMono(org.sp.Person.class);
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
...
----
	Mono<org.sp.Person> result = client.get()
			.uri("/persons/{id}", id).accept(MediaType.APPLICATION_JSON)
			.exchange()
			.flatMap(response -> response.bodyToMono(org.sp.Person.class));
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
...
.Java
----
	Mono<ResponseEntity<org.sp.Person>> result = client.get()
			.uri("/persons/{id}", id).accept(MediaType.APPLICATION_JSON)
			.exchange()
			.flatMap(response -> response.toEntity(org.sp.Person.class));
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
...
.Java
----
	Mono<org.sp.Person> personMono = ... ;

	Mono<Void> result = client.post()
			.uri("/persons/{id}", id)
			.contentType(MediaType.APPLICATION_JSON)
			.body(personMono, org.sp.Person.class)
			.retrieve()
...
.Kotlin
----
	val personDeferred: Deferred<org.sp.Person> = ...

	client.post()
			.uri("/persons/{id}", id)
			.contentType(MediaType.APPLICATION_JSON)
			.body<org.sp.Person>(personDeferred)
			.retrieve()
			.awaitBody<Unit>()
----
...
.Java
----
	Flux<org.sp.Person> personFlux = ... ;

	Mono<Void> result = client.post()
			.uri("/persons/{id}", id)
			.contentType(MediaType.APPLICATION_STREAM_JSON)
			.body(personFlux, org.sp.Person.class)
			.retrieve()
			.bodyToMono(Void.class);
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
...
----
	...
	builder.part("jsonPart", new org.sp.Person("Jason"));
...
----
	val builder = MultipartBodyBuilder().apply {
		...
		part("jsonPart", new org.sp.Person("Jason"))
...
.Java
----
	org.sp.Person person = client.get().uri("/person/{id}", i).retrieve()
		.bodyToMono(org.sp.Person.class)
		.block();

	List<org.sp.Person> persons = client.get().uri("/persons").retrieve()
		.bodyToFlux(org.sp.Person.class)
		.collectList()
		.block();
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
...
.Java
----
	Mono<org.sp.Person> personMono = client.get().uri("/person/{id}", personId)
			.retrieve().bodyToMono(org.sp.Person.class);
	...
	val data = runBlocking {
			val personDeferred = async {
				client.get().uri("/person/{id}", personId)
						.retrieve().awaitBody<org.sp.Person>()
			}
