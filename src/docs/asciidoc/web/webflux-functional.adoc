...
The following example extracts the body to a `Flux<org.sp.Person>` (or a `Flow<org.sp.Person>` in Kotlin),
where `org.sp.Person` objects are decoded from someserialized form, such as JSON or XML:

[source,java,role="primary"]
.Java
----
Flux<org.sp.Person> people = request.bodyToFlux(org.sp.Person.class);
----
[source,kotlin,role="secondary"]
.Kotlin
----
val people = request.bodyToFlow<org.sp.Person>()
----
...
Flux<org.sp.Person> people = request.body(BodyExtractors.toFlux(org.sp.Person.class));
----
[source,kotlin,role="secondary"]
.Kotlin
----
	val string = request.body(BodyExtractors.toMono(String::class.java)).awaitFirst()
	val people = request.body(BodyExtractors.toFlux(org.sp.Person::class.java)).asFlow()
----
...
.Java
----
Mono<org.sp.Person> person = ...
ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(person, org.sp.Person.class);
----
[source,kotlin,role="secondary"]
.Kotlin
----
val person: org.sp.Person = ...
...
For example, the following class exposes a reactive `org.sp.Person` repository:
...
public class PersonHandler {
	...
	public Mono<ServerResponse> listPeople(ServerRequest request) { // <1>
		Flux<org.sp.Person> people = repository.allPeople();
		return ok().contentType(APPLICATION_JSON).body(people, org.sp.Person.class);
	}

	public Mono<ServerResponse> createPerson(ServerRequest request) { // <2>
		Mono<org.sp.Person> person = request.bodyToMono(org.sp.Person.class);
...
----
<1> `listPeople` is a handler function that returns all `org.sp.Person` objects found in the repository as
JSON.
<2> `createPerson` is a handler function that stores a new `org.sp.Person` contained in the request body.
Note that `PersonRepository.savePerson(org.sp.Person)` returns `Mono<Void>`: an empty `Mono` that emits
...
when the `org.sp.Person` has been saved).
<3> `getPerson` is a handler function that returns a single person, identified by the `id` path
variable. We retrieve that `org.sp.Person` from the repository and create a JSON response, if it is
...
----
	class PersonHandler(private val repository: PersonRepository) {
		...
		suspend fun listPeople(request: ServerRequest): ServerResponse { // <1>
			val people: Flow<org.sp.Person> = repository.allPeople()
			return ok().contentType(APPLICATION_JSON).bodyAndAwait(people);
		}

		suspend fun createPerson(request: ServerRequest): ServerResponse { // <2>
			val person = request.awaitBody<org.sp.Person>()
...
JSON.
<2> `createPerson` is a handler function that stores a new `org.sp.Person` contained in the request body.
Note that `PersonRepository.savePerson(org.sp.Person)` is a suspending function with no return type.
<3> `getPerson` is a handler function that returns a single person, identified by the `id` path
variable. We retrieve that `org.sp.Person` from the repository and create a JSON response, if it is
...
<<core.adoc#validation, Validator>> implementation for a `org.sp.Person`:
...
----
	public class PersonHandler {
		...
		public Mono<ServerResponse> createPerson(ServerRequest request) {
			Mono<org.sp.Person> person = request.bodyToMono(org.sp.Person.class).doOnNext(this::validate); // <2>
			return ok().build(repository.savePerson(person));
		}

		private void validate(org.sp.Person person) {
...
----
	class PersonHandler(private val repository: PersonRepository) {
		...
		private fun validate(person: org.sp.Person) {
