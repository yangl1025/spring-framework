...
----
	client.get().uri("/persons")
			.exchange()
			.expectStatus().isOk()
			.expectBodyList(org.sp.Person.class).hasSize(3).contains(person);
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	...
	client.get().uri("/persons")
			.exchange()
			.expectStatus().isOk()
			.expectBodyList<org.sp.Person>().hasSize(3).contains(person)
----
...
----
	...
	client.get().uri("/persons/1")
			.exchange()
			.expectStatus().isOk()
			.expectBody(org.sp.Person.class)
			.consumeWith(result -> {
...
.Java
----
	EntityExchangeResult<org.sp.Person> result = client.get().uri("/persons/1")
			.exchange()
			.expectStatus().isOk()
			.expectBody(org.sp.Person.class)
			.returnResult();
