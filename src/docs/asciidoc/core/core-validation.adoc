...
.Kotlin
----
	class org.sp.Person(val name: String, val age: Int)
----

The next example provides validation behavior for the `org.sp.Person` class by implementing the
...
example implements `Validator` for `org.sp.Person` instances:
...
----
	public class PersonValidator implements Validator {
		...
		/**
		 * This Validator validates only org.sp.Person instances
		 */
		public boolean supports(Class clazz) {
			return org.sp.Person.class.equals(clazz);
		}

		public void validate(Object obj, Errors e) {
			ValidationUtils.rejectIfEmpty(e, "name", "name.empty");
			org.sp.Person p = (org.sp.Person) obj;
			if (p.getAge() < 0) {
...
----
	class PersonValidator : Validator {
		...
		override fun supports(clazz: Class<*>): Boolean {
			return org.sp.Person::class.java == clazz
		}

		override fun validate(obj: Any, e: Errors) {
			ValidationUtils.rejectIfEmpty(e, "name", "name.empty")
			val p = obj as org.sp.Person
			if (p.age < 0) {
				e.rejectValue("age", "negativevalue")
			} else if (p.age > 110) {
				e.rejectValue("age", "too.darn.old")
